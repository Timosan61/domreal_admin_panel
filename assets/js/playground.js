...
async function loadCalls() {
    ...
    try {
        const response = await fetch(`api/playground_calls.php?date=${date}&limit=${limit}`);
...
async function startAnalysis() {
    ...
    try {
        const response = await fetch('api/playground_analyze.php', {
            method: 'POST',
...
function startProgressTracking(date, total) {
    ...
    progressInterval = setInterval(async () => {
        ...
        try {
            const response = await fetch(`api/playground_progress.php?task_id=${analysisTaskId}&date=${date}&total=${total}`);
            const data = await response.json();
...
async function loadResults() {
    ...
    try {
        const response = await fetch(`api/playground_results.php?call_ids=${callIds}&models=${models.join(',')}`);
        const data = await response.json();
        ...
        updateStatus('Ошибка загрузки результатов: ' + error.message, 'danger');
    }
}
