<?php
/**
 * API для получения результативности отделов
 * GET /api/analytics/departments.php
 *
 * Параметры:
 ...
 * - limit: количество топ отделов (по умолчанию 10)
 */
...
// Запрос результативности отделов
$query = "
    SELECT
        cr.department,
        ...
        ROUND(
            CASE
                WHEN COUNT(DISTINCT cr.callid) > 0
                THEN (COUNT(DISTINCT CASE
                    WHEN LOWER(ar.call_result) LIKE '%показ%'
                    THEN cr.callid
                END) * 100.0 / COUNT(DISTINCT cr.callid))
                ELSE 0
            END,
            1
        ) as showing_rate,
