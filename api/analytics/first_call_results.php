<?php
/**
 * API для получения распределения результатов первого звонка по менеджерам
 * GET /api/analytics/first_call_results.php
 *
 * Параметры:
...
// Базовый WHERE с датами и фильтром первого звонка
$where_conditions = [
    "cr.started_at_utc >= :date_from",
    "cr.started_at_utc < DATE_ADD(:date_to, INTERVAL 1 DAY)",
    "cr.is_first_call = 1"  // Только первые звонки
];
...
// Запрос распределения результатов первого звонка по менеджерам
$query = "
    SELECT
